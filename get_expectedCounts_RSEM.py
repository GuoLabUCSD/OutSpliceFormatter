...
args_path.add_argument('-d', '--rsem_results_normals', type=str, required=True, help='Path to Normal Directory with genes.results files from RSEM')
...
def combine_rsem_rawcounts(rsem_results_tumors, rsem_results_normals):
    ...
    for file in os.listdir(rsem_results_normals):
        rsem_results_df = pd.DataFrame()
        if file.endswith("genes.results"):
            path = os.path.join(rsem_results_normals, file)
            results = pd.read_csv(path, sep = '\t')
            rsem_results_df = rsem_results_df.append(results)
            rsem_results_df = rsem_results_df.drop(['transcript_id(s)', 'length', 'effective_length', 'TPM', 'FPKM'], axis = 1)
            filename = file.split('.genes.')[0]
...
X = combine_rsem_rawcounts(args.rsem_results_tumors, args.rsem_results_normals)
